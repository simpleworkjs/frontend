...
(function($){
  ...
  // Show/hide one row.
  //
  // NOT jQuery's .hide(): that writes a plain inline `display: none`, which
  // loses to Bootstrap's display utilities (`.d-flex`, `.d-block`, `.d-grid`
  // …) because those are declared `!important` in the stylesheet, and an
  // `!important` rule beats a non-important inline one. Rows carrying such a
  // class — list-group items, card grids, most flex layouts — simply stayed
  // visible while the filter's own count said they were hidden.
  //
  // Writing our own `!important` inline wins that fight; on the way back we
  // restore whatever inline display the view had, so the element falls back to
  // its class exactly as before we touched it.
  //
  // Deliberately not jQuery's .stop() either: cancelling in-flight animations
  // would jump a view's `take` hook (commonly fadeOut(500, remove)) straight
  // to its removal callback on any unrelated refresh.
  function setRowVisible(node, visible){
    if (!node || !node.style) return;
    if (!visible) {
      if (!node.__swFilterHidden) {
        node.__swFilterPrev = {
          value: node.style.getPropertyValue('display'),
          priority: node.style.getPropertyPriority('display'),
        };
        node.__swFilterHidden = true;
      }
      node.style.setProperty('display', 'none', 'important');
      return;
    }
    // Only un-hide rows this filter hid; never override a view that is hiding
    // a row for its own reasons.
    if (!node.__swFilterHidden) return;
    const prev = node.__swFilterPrev || {value: '', priority: ''};
    node.style.removeProperty('display');
    if (prev.value) node.style.setProperty('display', prev.value, prev.priority);
    node.__swFilterHidden = false;
  }
    ...
    bind: function(scopeName, options){
      ...
      function applyClient(){
        ...
        for (const row of scope) {
          ...
          if ($el && $el.length) setRowVisible($el[0], visible);
        }
        return shown;
      }
      ...
      function updateCount(shownOverride){
        ...
        const shown = shownOverride != null ? shownOverride
          : serverMode ? (scope ? scope.length : 0)
            : applyClientCount();
        // In client mode the browser holds every row, so the scope *is* the
        // whole set — using the server's reported total would go stale the
        // moment a row arrived live or was deleted ("6 of 4 shown"). Only in
        // server mode, where the browser holds one page, is `total`
        // authoritative.
        const of = serverMode && total != null ? total
