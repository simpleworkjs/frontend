...
(function($){
  ...
  app.util = {
    ...
    // background, so the user's eye lands on the row that changed instead
    // of it silently appearing/updating somewhere off-screen. Takes a
    // jQuery object or a raw DOM node (e.g. jq-repeat's `item.__jq_$el`).
    // Uses inline styles + a Bootstrap 5 CSS variable rather than a
    // stylesheet class, since this package ships no CSS of its own.
    revealItem: function(el){
