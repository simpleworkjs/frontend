  ...
  app.sync = {
    ...
    bind: function(scopeName, modelName, options){
      ...
      function applyRow(scope, key, pk, record){
        ...
        // The record is authoritative about its own identity; the topic's pk is
        // the fallback. Publishers get this wrong in ways that are invisible
        // until a row duplicates — theta42's ModelPs, for one, published the
        // *class* name as the pk for every model keyed on `name`, because a
        // class always has a built-in `.name`. Trusting the topic there would
        // append a second copy of a record on every update instead of patching
        // the one already on screen.
        const id = row[key] !== undefined ? row[key] : pk;
