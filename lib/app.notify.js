...
(function($){
	...
	app.notify = {
		...
		push: function(payload, topic){
			...
			this.unread++;
			this.render();
			const collapsed = collapse([event])[0];
			this.showDesktop(collapsed);
			this.showToast(collapsed);
		},
		...
		render: function(){
			...
			const groups = collapse(this.events, config.maxRows).slice(0, config.maxRows);
			if (!groups.length) {
